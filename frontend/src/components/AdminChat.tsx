...
const AdminChat: React.FC<AdminChatProps> = ({ isOpen, onClose }) => {
  ...
  const loadAvailableUsers = async () => {
    try {
      ...
      // Filter to show only pharmacists for admin, or only admin for pharmacists
        ...
        <div className="w-1/3 border-r border-gray-200 flex flex-col">
          <div className="p-4 border-b border-gray-200 flex justify-between items-center bg-blue-50">
            <h3 className="text-lg font-semibold text-gray-900">
              {user?.role === 'ADMIN' ? 'Admin Chat' : 'Pharmacist Chat'}
            </h3>
            <button
              onClick={onClose}
          ...
          <div className="flex-1 overflow-y-auto">
            {isLoading ? (
              ...
              <div className="p-4 text-center text-gray-500">
                No {user?.role === 'ADMIN' ? 'pharmacists' : 'admins'} available to chat
              </div>
            ) : (
        ...
        <div className="flex-1 flex flex-col">
          {selectedUser ? (
            <>
              ...
              {/* Message Input */}
              <form onSubmit={handleSendMessage} className="p-4 border-t border-gray-200 bg-blue-50">
            ...
            <div className="flex-1 flex items-center justify-center text-gray-500">
              Select a {user?.role === 'ADMIN' ? 'pharmacist' : 'admin'} to start chatting
            </div>
          )}
        </div>
      </div>
